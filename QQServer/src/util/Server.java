...
public class Server {
	...
	*/
	...
	/*
	public boolean checkLoginedUser(String userName){	//check if this user is logined
	...
	*/
	...
	public void sendToAll(String message) {
		...
		for (Map.Entry<String,User> user : users.entrySet()) {
			...
			try {
				user.getValue().out.writeObject(new Message("server",user.getValue().userName,message));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void sendToAll(String from,String message) {
		...
		for (Map.Entry<String,User> user : users.entrySet()) {
			//entry.getValue().out.println("server : "+message);
			try {
				user.getValue().out.writeObject(new Message(from,user.getValue().userName,message));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void relay(Message message) {
		...
		if(!checkLoginedUser(message.to)) {
			...
			try {
				to.out.writeObject(new Message("server",message.from,"User "+message.to+" not exists"));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		try {
			to.out.writeObject(new Message("(private message) from "+message.from,message.to,message.message));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
